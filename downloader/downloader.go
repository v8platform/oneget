...
func (dr *Downloader) downloadFile(fileToDownload *FileToDownload) (os.FileInfo, bool) {
	...
	fullpath := filepath.Join(dr.BasePath, fileToDownload.path,  fileToDownload.name)
	...
	if os.IsExist(err) {
		...
		err = os.MkdirAll(filepath.Join(dr.BasePath, strings.ToLower(fileToDownload.path)), os.ModeDir)
		if err != nil {
			return nil, false
		}
